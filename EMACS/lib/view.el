...
(defvar view-undesirable-functions 
  '(self-insert-command))
(defun view-make-undefined (key)
  (define-key view-mode-map key 'View-undefined))
(defun view-make-unavailable (f)
  (mapcar 'view-make-undefined (where-is-internal f)))

(defvar view-mode-map nil)
(if view-mode-map
    nil
  (setq view-mode-map (make-keymap))
  (mapcar 'view-make-unavailable view-undesirable-functions)
  ;;(define-key view-mode-map "\C-c" 'view-exit)
  (define-key view-mode-map "q" (function (lambda nil (interactive) (kill-buffer (current-buffer)))))
  (define-key view-mode-map "b" 'bury-buffer)
  ...
  (define-key view-mode-map "\C-x\C-q" 'view-exit)
  )
...
(defvar View-end-message "End.  Type \\[view-exit] to quit viewing.")
(defvar View-beg-message "Top.  Type \\[view-exit] to quit viewing.")
...
Arg is number of lines to scroll."
  ...
  (setq lines
	(if lines (prefix-numeric-value lines)
	  (view-scroll-size)))
  (cond ((and (pos-visible-in-window-p (point-max)) (>= lines 0))
	 (message (substitute-command-keys View-end-message)))
	((and (pos-visible-in-window-p (point-min)) (< lines 0))
	 (message (substitute-command-keys View-beg-message)))
	(t 
	 ;; (view-last-command 'View-scroll-lines-forward lines)
	 (cond ((>= lines (view-window-size))
		(scroll-up nil))
	       ((>= (- lines) (view-window-size))
		(scroll-down nil))
	       (t 
		(scroll-up lines)))
